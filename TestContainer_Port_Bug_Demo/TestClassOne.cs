...
namespace TestContainer_Port_Bug_Demo_IntegrationTests
{
    [TestFixture]
    public class TestClassOne : BaseTest
    {
        ...
        [Test]
        public async Task DummyTestMethodOne()
        {
            // Verify the connection by executing a simple query
            bool isConnected = await VerifyConnection();
            Assert.IsTrue(isConnected, "Connection to PostgreSQL database failed.");
        }
    }
}
